...
class RegulatoryCollector(BaseCollector):
    ...
    미국(SEC, CFTC, Fed), 한국(금융위), 아시아(FSA, MAS),
    유럽(ESMA, FCA) 등 글로벌 규제 기관의 뉴스를 수집하고
    종합 리포트를 생성합니다.
    """

    name = "regulatory"
    category = "regulatory-news"
    state_file = "regulatory_news_seen.json"
    ...
    def fetch(self) -> List[Dict[str, Any]]:
        ...
        return us_items + korea_items + asia_items + europe_items

    def process(self, items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """번역 및 URL 기반 중복 제거."""
        enrich_items(items, context_map=_SOURCE_CONTEXT, fetch_url=False)
    ...
    def run(self) -> None:
        """메인 실행 파이프라인 — 글로벌 규제 동향 리포트 생성."""
        self.logger.info("=== Starting regulatory news collection ===")
        ...
        if self.is_duplicate_exact(post_title, "consolidated"):
            self.logger.info("Regulatory post already exists, skipping")
            self.save_state()
        ...
        try:
            ...
            if card_themes:
                ...
                if img:
                    briefing_image = img
                    self.logger.info("Generated regulatory briefing card")
        except ImportError:
            pass
        except Exception as e:
            self.logger.warning("Regulatory briefing card failed: %s", e)
        ...
        if filepath:
            ...
            self.logger.info("Created regulatory news post: %s", filepath)

        self.save_state()
        self.logger.info(
            "=== Regulatory news collection complete: %d posts created ===",
            self._created_count,
        )
        self.log_summary(all_items)
