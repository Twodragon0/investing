"""generate_daily_summary 섹션 빌더 + Korean-text/감성/관계 헬퍼.

`generate_daily_summary.py` 에서 추출(2026-06-26). 일일 요약 본문을 구성하는
5개 섹션 빌더(`_build_market_signal_section`, `_build_snapshot_table`,
`_build_overview_section`, `_build_briefing_section`,
`_build_priority_and_category_sections`)와 이들이 의존하는 감성 분석·관계 추출·
한국어 표기 정리 leaf 헬퍼를 한 모듈로 모은다. 이 모듈은 closure 가 닫혀 있어
(메인 오케스트레이션/IO 함수를 호출하지 않음) 단방향 import 만 한다.
[[summary-post-categorizers]] [[summary-post-parsing]] 와 함께 메인 모듈을 슬림화한다.

메인 모듈(`generate_daily_summary.py`)과 테스트는 `gds.<name>` 으로 이 심볼들을
참조하므로 그쪽에서 재-export 된다.
"""
