...
> **작성일**: 2026-04-20 | **상태**: Phase 1·3 구현 완료(2026-05 추정), 라인 참조 동기화 2026-05-22 | **범위**: `scripts/common/summarizer.py` 의 `_assess_risk_level` / `classify_priority` / P0 경보 트리거 전반
...
> **구현 상태 업데이트(2026-05-22)**
> - **Phase 1 (모듈 뼈대)**: ✅ `scripts/common/risk_classifier.py` 신규 (612줄, `RiskSignals` / `ItemScore` / `RiskVerdict` / `extract_signals` / `score_item` / `apply_overrides` / `classify_risk`).
> - **Phase 3 (summarizer 연결)**: ✅ `summarizer.py:656` `_assess_risk_level` 이 `from .risk_classifier import classify_risk` 로 위임. `self._last_risk_verdict` 보관 + `logger.info("risk_level=…")` 트레이스.
> - **단위 테스트**: ✅ `tests/test_risk_classifier.py` (560줄), `tests/test_summarizer.py:703-766` 6개 verdict 기반.
> - **`score_impact()` 활성화 여부**: ❌ 미사용 유지. Phase 1 은 `score_impact` 의 정규식/가중치를 **그대로 호출하지 않고** `risk_classifier` 에 독립적으로 재구현. 따라서 `score_impact` 는 `tests/test_summarizer.py::TestScoreImpact` 6 케이스로 계약만 잠긴 **legacy 자산**이며, 본 문서의 "재사용 자산" 표현은 *역사적 의도*이지 현재 구현이 아니다.
> - **남은 단계**: Phase 2 threshold 튜닝 / Phase 4 회귀 fixture / Phase 5 로깅 분석 / Phase 6 문서·배지 통합 — `docs/p4-threshold-validation-30-day-distribution-analysis.md` 참고.
...
→ `scripts/common/summarizer.py:813` `generate_overall_summary_section()` 이 `risk_level` 을 문자열 포맷팅
→ `scripts/common/summarizer.py:809` `risk_level = self._assess_risk_level(priority_items)`
→ `scripts/common/summarizer.py:656-676` `_assess_risk_level()`

```python
# scripts/common/summarizer.py:656-676
...
`priority_items` 는 `classify_priority()` 결과(`summarizer_priority.py:131-187`). 판정 기준은 **P0/P1 키워드 텍스트 매칭** 단 하나.

```python
# scripts/common/summarizer_priority.py:146-160 (요약)
...
### 1-2. P0 키워드(`summarizer_priority.py:19-39`)
...
|------|------|---------------------------|
| `summarizer.py:809-813` | `### 전체 뉴스 요약` 블록 하단 배너 | 2026-04-20 포스트 71행 |
| `summarizer.py:819-826` | `### 긴급 이슈` 리스트 3건 | 포스트 73-77행 (개인 발언이 1순위) |
| `summarizer.py:961-988` | `한눈에 보기` 통계 그리드 `🔴 높음` | 포스트 40행 |
| `summarizer.py:1062-1097` | `<div class="alert-box alert-urgent">` 긴급 알림 박스 | 포스트 52-59행 |
| `summarizer.py:1122-1146` | `투자자 인사이트` 리스크 평가 | 하단 |
...
|------|------|-------------|---------------|
| `score_impact()` — 소스 가중치 + 콘텐츠 시그널(%, $bn, 기관명, urgency) | `summarizer.py:1220-1255` | **production 호출 없음**(2026-05 기준; `risk_classifier` 가 독립 재구현으로 대체) | **재사용 안 됨** — 문서적 legacy |
| `_SOURCE_WEIGHTS` dict | `summarizer.py:1206-1218` | `score_impact()` 내부만 | **높음** |
| `_classify_source()` 6-type 분류 | `common/markdown_utils.py:194-235` | 배지 색상 | **중간** — 매핑 재사용 |
| `_SENTIMENT_POS / _SENTIMENT_NEG` | `summarizer.py:1257-1303` | `get_theme_sentiment()` | **중간** — 기사 단위 sentiment |
| `_classify_news_severity()` | `severity.py:60-67` | 개별 뉴스 카드 배지 | **중간** — 독립 경로 |
...
현재 시스템은 **오탐 축 전체(F1~F6)에 대해 방어 없음**. 정탐 축 중 T1만 `score_impact()` 에 부분 반영(`summarizer.py:1240-1244` `%` 와 `$billion` 정규식) — 그러나 `score_impact` 는 호출되지 않음.
...
|------|-----------|-----------|-------------|------|
| **S1. 소스 신뢰도** | `_SOURCE_WEIGHTS` (`summarizer.py:1206`), `_classify_source` (`markdown_utils.py:194`) | 즉시 | ×0.4 ~ ×1.5 | 6-type(regulator/crypto-media/finance-media/world-media/exchange/aggregator) 매핑 |
| **S2. 구체적 금액/수치** | `score_impact` (`summarizer.py:1240-1244`) 정규식 재사용 | 즉시 | +2 | `\$[\d,.]+\s*(million\|billion\|B\|M)`, `[+-]?\d+\.?\d*%` |
| **S3. 기관/규제 엔티티** | `entity_extractor._ORG_ENTITIES` (`entity_extractor.py:56-64`), `score_impact` institutions 리스트 (`summarizer.py:1247`) | 즉시 | +1.5 | SEC/Fed/ECB/금감원 등 언급 시 |
| **S4. 시장 메커니즘 발동 키워드** | `PRIORITY_KEYWORDS["P0"]` 의 시장-구조 부분집합 | 즉시 | +2 | circuit breaker / halt / bank run / default / bankruptcy |
| **S5. 오피니언/개인 발언 마커** | `_SEVERITY_LOW_KW` (`severity.py:44-58`) + 신규 패턴 | 즉시 | **−3 (차감)** | "says"/"말합니다"/"opinion"/"prediction"/"인터뷰"/"주장" |
| **S6. 엔터테인먼트/인물 기반 부정** | `content_filters._DEFAULT_ENTERTAINMENT_KEYWORDS` (`content_filters.py:16`) | 즉시 | 차감 | 이미 존재 |
| **S7. Sentiment 방향성** | `_SENTIMENT_POS/NEG` (`summarizer.py:1257-1303`) | 즉시 | −1.5 (긍정 시) | 긍정 키워드 과반이면 CRITICAL 부적합 |
...
**장점**
- `score_impact()`(`summarizer.py:1220-1255`) 의 **기존 뼈대 재사용** → 죽은 코드를 살림
...
P0 렌더링(`summarizer.py:819-826`, `1062-1097`)도 **점수 내림차순**으로 정렬된 `verdict.top_items` 를 사용하도록 변경 → 2026-04-20 포스트의 "*O.C. Guy*" 가 자연히 하단으로 밀려남.
...
- [ ] 기존 테스트 `tests/test_summarizer.py:703-766` 는 새 verdict 기반으로 재작성(6개 테스트)
...
| R3 | `score_impact` 의 `_SOURCE_WEIGHTS` 가 구버전 소스명만 커버 (`summarizer.py:1206-1218`) | 중 | Phase 1 에서 `_classify_source` 기반 6-type 매핑으로 교체, 소스별 weight dict 리팩터 |
...
| R9 | `_SEVERITY_HIGH_KW` (개별 뉴스 카드 배지, `severity.py:8-44`) 와 새 판정의 **출력 불일치** | 중 | Phase 6 에서 배지 레벨도 `item.score` 기반으로 ≥7 = high / ≥4 = medium / else = low 매핑 |
...
### 현재 로직
- `scripts/common/summarizer.py:656-676` — `_assess_risk_level` 본체 (CRITICAL 임계값 P0≥3)
- `scripts/common/summarizer_priority.py:131-187` — `classify_priority` 키워드 `kw in text` 매칭
- `scripts/common/summarizer_priority.py:18-106` — `PRIORITY_KEYWORDS` P0/P1/P2 사전
- `scripts/common/severity.py:8-67` — `_SEVERITY_HIGH_KW`, `_SEVERITY_LOW_KW`, `_classify_news_severity`
- `scripts/common/summarizer.py:357-362` — `RISK_LEVELS` 한국어 설명
- `scripts/common/summarizer.py:813` — `**리스크 수준 [{risk_level.upper()}]**` 렌더링 라인
- `scripts/common/summarizer.py:1062-1097` — P0 "긴급 알림" 박스 렌더링

### 재사용 가능 자산
- `scripts/common/summarizer.py:1206-1218` — `_SOURCE_WEIGHTS`
- `scripts/common/summarizer.py:1220-1255` — `score_impact()` (production 호출 없음; `risk_classifier.score_item` 이 실질 대체)
- `scripts/common/risk_classifier.py` — Phase 1·3 실구현 모듈 (RiskSignals/ItemScore/RiskVerdict + classify_risk)
- `scripts/common/summarizer.py:1257-1303` — `_SENTIMENT_POS`, `_SENTIMENT_NEG`
- `scripts/common/summarizer.py:1305-1320` — `get_theme_sentiment`
...
### 기존 테스트 (보존/확장 대상)
- `tests/test_summarizer.py:703-766` — `_assess_risk_level` 6개 케이스(현재 3건 이상 P0 → critical 기대)
- `tests/test_summarizer_extended.py:783-820` — `classify_priority` dedup 추가 커버리지 4개
