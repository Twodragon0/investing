...
**목표 분포** (최근 30일 crypto-news-digest 60 포스트 기준):

| 레벨 | 목표 비율 | 현재 비율(추정) | 근거 |
|------|-----------|------------------|------|
| critical | 1~3 % | 7% (4/60) | 실제 주요 이벤트 빈도 |
| elevated | 10~20 % | 추정 25% | P0 1건에서 즉시 elevated 는 과민 |
| moderate | 30~50 % | 추정 40% | - |
| low | 30~50 % | 추정 28% | 평시 |

**튜닝 절차**: `scripts/tools/tune_risk_threshold.py`(신규 선택) 가 과거 30일 포스트를 파싱해 점수 분포를 내고, CRITICAL≈2%가 되는 `CRITICAL_MEAN_TOP_3` 를 이분탐색. 최소 `P0_THRESHOLD` 는 "기관 + 금액"만으로 도달하도록(=`2.0 × source_base(2.5) × 0.5 clip + 2.0 amount = 5.0`) 설계.
