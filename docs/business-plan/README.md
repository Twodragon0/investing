...
| **Quant Developer** | CPCV 교차검증, Monte Carlo 시뮬레이션, DSR 과적합 검정, IC/ICIR 분석, 동적 레짐 감지 | 백테스트 보고서 다수, 2,373개 자동화 테스트 |
...
| 백테스팅 엔진 | **검증 완료** | DSR, Monte Carlo, IC/ICIR, CVaR, CPCV 검증 (2,373 tests) |
...
```
Exp Drawdown Scaling: exp(-10 * dd), 임계값 2%, 바닥 10%
    ↓ 낙폭 증가 시
Vol-of-Vol Penalty: 변동성 클러스터링 감지 (CV > 0.5 → 포지션 축소)
    ↓
Time-Decay Exit: 48캔들 이후 점진적 수익 요구 (0% → 2%)
    ↓
Circuit Breaker (일일 MDD 20%)
    ↓ 초과 시
자동 거래 중단 → 포지션 청산 → Slack 알림
    ↓ 회복 시
점진적 포지션 복원 (동적 레짐 감지로 재진입 조건 판단)
```
