...
단일 지표가 아닌, **14개 마켓 인텔리전스 컴포넌트 + 7개 기술적 지표**를 합성하여 시장 국면에 적응하는 복합 시그널을 생성합니다.

| 시그널 계층 | 구성 | 가중치 |
|:-----------|:-----|:------|
| **기술적 분석 (TA)** | EMA, MACD, RSI, Bollinger, ATR, OBV, VWAP | 75% |
| **마켓 인텔리전스 (MI)** | Fear & Greed, 소셜 감성, 매크로, 펀딩레이트 등 14개 | 25% |
| **리스크 제어** | Quarter-Kelly, CVaR, Drawdown Scaling, Vol-of-Vol, Circuit Breaker | 최종 필터 |
...
→ BUY if > threshold | SELL if < threshold (레짐별 동적 조정)
...
| CI/CD | 39개 GitHub Actions (23+16) | 무중단 자동 배포 |
...
| **DevSecOps** | CI/CD 39개 워크플로우 설계/운영, OWASP Top 10 준수, 컨테이너 보안 | Bandit/Trivy/CodeQL 자동화 |
| **Quant Developer** | CPCV 교차검증, Monte Carlo 시뮬레이션, DSR 과적합 검정, IC/ICIR 분석, 동적 레짐 감지 | 백테스트 보고서 다수, **2,485개** 자동화 테스트 |
...
| Infra | AWS, GitHub Actions (39 workflows), Sentry, Slack |
...
| 구성 요소 | 상태 | URL/위치 |
|:----------|:-----|:---------|
| 데이터 수집 파이프라인 | **운영 중** | [investing repo](https://github.com/Twodragon0/investing) - 23개 워크플로우 |
...
| 백테스팅 엔진 | **검증 완료** | DSR, Monte Carlo, IC/ICIR, CVaR, CPCV 검증 |
| FastAPI 대시보드 | **개발 완료** | REST API + WebSocket 실시간 |
| 자동화 테스트 | **운영 중** | investing 112 + crypto 2,373 = **2,485개** |

### 실시간 대시보드 상세 기능

FastAPI + WebSocket 기반 5탭 실시간 대시보드를 자체 개발하였습니다.

| 탭 | 기능 | 실시간 데이터 |
|:---|:-----|:------------|
| **시장 (Market)** | BTC/ETH 시세, Fear & Greed, 24h 거래량, BTC 점유율, 시가총액, Upbit TOP 10, 시장 점유율 차트, 글로벌 변동성, DeFi TVL, 스윙 트레이딩 지표 | WebSocket 실시간 갱신 |
| **뉴스 (News)** | 국내/해외 뉴스 분리, 보안 뉴스(Rekt News, DeFi Llama) | 자동 수집 + 분류 |
| **알림 (Alerts)** | 심각도별 필터(CRITICAL~INFO), 소스별 분포 차트, 24h 타임라인, 가동시간 모니터링 | 실시간 알림 푸시 |
| **리포트 (Reports)** | 시장/뉴스/보안/경제/내부자 카테고리 필터, Markdown 렌더링, 상세 모달 | 일일 자동 생성 |
| **트레이딩 (Trading)** | 거래 모드(Paper/Live), 오늘 거래 수, 승률, 총 평가액, 매매일지, 현재 포지션 | ATS 3.0 연동 |

**기술 스택**: Chart.js 차트, Lucide 아이콘, CSP nonce 보안, skeleton UI 로딩, 반응형 모바일 지원
...
|:---------|:---------------|:-----------|
| 시그널 소스 | RSI, MACD 등 1-2개 | **14 MI + 7 TA = 21개** |
| 리스크 관리 | 단순 손절 | Kelly + CVaR + Drawdown Scaling + Vol-of-Vol |
| 뉴스 반영 | 수동 또는 미반영 | **자동 수집 → 감성 분석 → 시그널** |
| 보안 | API Key 평문 저장 | DevSecOps + IAM 최소 권한 |
| 검증 | 단순 백테스트 | DSR + Monte Carlo + IC/ICIR + CPCV |
...
기존 자동매매 봇의 "단순 손절" 방식과 달리, DragonQuant는 **5개 계층의 독립적 리스크 제어 모듈**이 순차적으로 작동합니다. 각 모듈은 실제 코드에 구현되어 운영 중입니다.

```
① Dynamic Regime Detection (시장 국면 감지)
   ADX + EMA 기반 Bull/Bear/Sideways 분류
   신뢰도 > 30% 시 매매 임계값 동적 조정
    ↓
② Exp Drawdown Scaling (지수적 낙폭 축소)
   포지션 비중 = exp(-10 × 현재 낙폭)
   낙폭 2% → 포지션 82%, 5% → 60%, 10% → 37%
   바닥: 최소 10% (완전 청산 방지)
    ↓
③ Vol-of-Vol Penalty (변동성 클러스터링 감지)
   최근 10캔들 변동성의 변동계수(CV) 측정
   CV > 0.5 → 포지션 최대 50% 강제 축소
   급변하는 시장에서 과도 노출 방지
    ↓
④ Time-Decay Exit (시간 기반 수익 요구)
   진입 후 48캔들 경과 시 수익률 요구 증가
   0% → 2% 점진적 요구, 미달 시 자동 청산
   "박스권 고착" 포지션 정리
    ↓
⑤ Circuit Breaker (서킷 브레이커)
   일일 MDD 20% 도달 시 자동 거래 중단
   모든 포지션 즉시 청산 + Slack 알림
   레짐 감지를 통해 회복 조건 충족 시 점진적 복원
```

#### 레짐별 매매 임계값 동적 조정

| 시장 국면 | 감지 방법 | 매수 임계값 | 매도 임계값 | 행동 패턴 |
|:---------|:---------|:----------|:----------|:---------|
| **Bull (상승장)** | ADX↑ + EMA 위 | 완화 (더 쉽게 매수) | 강화 (늦게 매도) | 추세 추종 극대화 |
| **Bear (하락장)** | ADX↑ + EMA 아래 | 강화 (매수 억제) | 완화 (빠르게 매도) | 자본 방어 우선 |
| **Sideways (횡보)** | ADX↓ | 기본값 유지 | 기본값 유지 | 과매매 방지 |

> **실제 효과**: Bear 레짐(Fear & Greed=12)에서 매수 시그널 발생 시, 임계값 강화로 매수를 억제하여 하락장 진입을 원천 차단. 이 메커니즘이 **MDD 6.93%** 달성의 핵심 요인입니다.

#### 기존 봇 vs DragonQuant 리스크 관리 상세 비교

| 리스크 시나리오 | 기존 자동매매 봇 | DragonQuant |
|:-------------|:-------------|:-----------|
| 갑작스런 10% 급락 | 손절 주문 실행 (슬리피지 위험) | ② Exp Drawdown → 포지션 37%로 자동 축소 |
| 변동성 급등 (뉴스 이벤트) | 대응 없음 (계속 매매) | ③ Vol-of-Vol → 50% 강제 축소 |
| 장기 횡보 (박스권 고착) | 수수료 누적 손실 | ④ Time-Decay → 48캔들 후 자동 청산 |
| 하락장 진입 | 계속 매수 (MDD 60%+) | ① Bear 레짐 감지 → 매수 임계값 강화 |
| 연쇄 하락 (MDD 20%+) | 원금 전액 노출 | ⑤ Circuit Breaker → 즉시 중단 + 알림 |

### D. 코드 품질 및 검증 체계

| 검증 항목 | 수량 | 도구 |
|:---------|:----|:-----|
| 자동화 테스트 | **2,485개** (investing 112 + crypto 2,373) | pytest |
| CI/CD 워크플로우 | **39개** (investing 23 + crypto 16) | GitHub Actions |
| 보안 스캔 | **5종** | Bandit, Trivy, CodeQL, Gitleaks, Safety |
| 코드 커버리지 | 핵심 모듈 90%+ | pytest-cov |
| 백테스트 검증 | **6종** | DSR, Monte Carlo, IC/ICIR, CVaR, CPCV, Profit Factor |

> 이 수준의 자동화 테스트와 보안 스캔 체계는 **스타트업이 아닌 금융기관급 품질 관리**에 해당합니다.
