...
3. **레코드가 아예 안 생기는 경우가 진짜 쿼터 거절이다.** 2026-08-07 main 커밋
   ...
   ~~거절은 15:54–17:56 구간에 12건 연속으로 몰렸고 그 앞뒤 커밋은 정상 생성됐다 —
   누적 총량이 아니라 시간 구간에 반응한다.~~ **이 문장은 2026-08-10 재측정에서
   틀린 것으로 확인됐다.** 실제 거절 구간은 12:48–19:38 이고 연속 블록이 아니라
   5개 블록으로 정상 생성과 인터리브됐다. 아래 "재측정" 절을 볼 것.
...
`collector-push-batching-design.md` 의 "남는 불확실성"이 물었던 것: 거절이 **하루
총량**에 반응하는가, **분당/시간당 생성률**에 반응하는가. 셋 다 아니었다 —
**수 시간~하루 폭의 롤링 윈도우 누적**이다.

측정: `vercel ls investing --environment {production,preview} -F json` 을 `--next`
로 끝까지 페이지네이션(production 800건 / preview 163건, 2026-07-18까지 소급) 한
뒤 `git log origin/main` 과 SHA 대조. 창은 08-04~08-10 13:14 KST, main 커밋 284건.

| | 건수 |
|---|---|
| Production 레코드 생성 | 257 |
| 푸시 batch non-head (5초 간격 쌍의 앞 커밋) | 3 |
| 레코드 없음 = 거절 | **24** |

일자별 거절: 08-04 0 / 08-05 1 / 08-06 0 / **08-07 22** / 08-08 0 / 08-09 1 /
08-10 0. 즉 거절의 92%가 하루에 몰려 있고, 나머지 2건은 부하가 낮은 시각의
단발이라 쿼터로 설명되지 않는다(08-05 23:30, 08-09 02:00). 이 2건은 창을 08-18
까지 넓히면 3건이 된다 — 아래 "쿼터로 설명되지 않는 단발 거절" 절을 볼 것.

**총량(달력일) 가설 — 기각.** 거절 이후 같은 날 안에서 레코드 생성이 재개된다:
08-07 은 첫 거절(12:48) 시점 누적 48건이었고 그 뒤로 24건이 더 생성됐다. 08-09 는
첫 거절 시점 누적 1건.

**생성률(단기) 가설 — 기각.** 직전 10분 레코드 수는 거절 커밋이 평균 **0.1**건,
정상 생성 커밋이 평균 0.5건으로 **거절 쪽이 오히려 낮다**. 30분/60분으로 넓혀도
평균은 역전되지만(1.9 vs 1.6, 4.7 vs 3.0) 정상 생성의 최대치가 거절의 최대치보다
높아 임계가 서지 않는다.

**롤링 누적 가설 — 채택.** (윈도우, 카운트 대상, 임계) 격자 탐색에서 Youden's J
최고 조합:

| 카운트 대상 | 윈도우 | 임계 | 거절 검출 | 정상 오탐 | J |
|---|---|---|---|---|---|
| production+preview 전 레코드 | **30h** | **106** | 22/24 | 8/257 | **0.886** |
| production 만 | 36h | 66 | 22/24 | 17/257 | 0.851 |
| 실제 빌드(`READY`)만 | 36h | 58 | 22/24 | 52/257 | 0.714 |

읽는 법 세 가지:

1. **`CANCELED` 레코드도 쿼터를 먹는다.** `READY` 만 세는 축이 가장 나쁘게
   갈린다(J 0.714, 오탐 52건). 위 "확정되는 것" 1번(`ignoreCommand` 는 빌드 시간만
   줄인다)이 쿼터 축에서도 재확인됐다.
2. **프리뷰를 합쳐 세는 축이 가장 잘 갈린다.** production 만으로는 J 가 떨어진다 —
   프리뷰 레코드도 같은 통에 들어간다.
3. **거절은 임계선 왕복으로 나타난다.** 08-07 의 시퀀스는
   `ok×24 REJ×3 ok×2 REJ×3 ok×2 REJ×13 ok×1 REJ×2 ok×1 REJ×1 ok×6` — 거절 블록
   5개가 정상 생성과 인터리브된다. 윈도우가 미끄러지며 카운트가 임계 아래로
   내려가면 1건이 통과하고 다시 막히는, 슬라이딩 윈도우 상한의 전형적 서명이다.
   24h 롤링으로 보면 08-07 12시에 **91**(전 기간 최대는 94)까지 올랐다가 거절이
   시작됐고, 76 아래로 빠진 20시 이후 거절이 멎었다.

**식별 한계.** 포화 사건이 **1회(08-07)뿐**이라 윈도우 폭과 임계는 강하게
식별되지 않는다. 24h/95, 30h/106, 36h/116 이 모두 비슷하게 들어맞는다. 확정되는
것은 **축의 종류**(단기 생성률 ✗ / 달력일 총량 ✗ / 롤링 누적 ✓)이지 그 숫자가
아니다. 두 번째 포화 사건이 나오기 전까지 임계를 상수로 코드에 박지 말 것.

**배칭 설계에 주는 답.** 레코드 총량을 줄이는 조치(`collector-push-batching-design.md`
의 옵션 A)는 **효과가 있다.** 구속 조건이 롤링 누적이므로 하루 레코드를 41% 줄이면
롤링 카운트도 같은 비율로 내려가고, 관측된 피크 94는 임계 추정 하한(~95)보다
아래로 내려간다. 설계 문서가 걱정했던 "피크 구간이 그대로면 거절도 그대로"는
피크가 순간 속도일 때만 성립하는데, 측정 결과 피크는 누적이었다.

재현: `vercel ls` 페이지네이션 → `git log origin/main --format='%H|%cI|%s'` → SHA
대조 → 90초 이내 후속 커밋이 레코드를 받았으면 non-head 로 분리 → 나머지가 거절.

#### 도구화와 재현 결과 (2026-08-14)

위 절차를 `scripts/tools/check_vercel_quota.py` 로 고정했다. 넓은 창(08-03~08-14)에서
돌린 뒤 이 절의 구간(08-04~파일럿)으로 잘라 대조하니 **main 커밋 284 / 레코드 생성
257 / non-head 3 / 거절 24(일자별 분포까지)** 가 전부 재현됐다. 절차는 유효하다.

손으로 훑은 탓에 생긴 수치 오차 두 개는 정정해 둔다:

- **"08-07 12시에 91"** — 12:00 시점의 24h 롤링은 **79** 이고, 91은 **13:00** 값이다.
  같은 문장의 "76 아래로 빠진 20시" 는 20:00 = 76 으로 정확하다. 서사(피크 직후 거절
  시작 → 76 아래에서 멎음)는 그대로 성립한다. 첫 거절은 12:48:19 다.
- **"전 기간 최대는 94"** — 도구 기준으로는 **95**(08-07 12:30:26.420)다. 손 절차가
  분/시 격자로 훑어 그 시각에 창을 놓지 못했다. 12:30:00 정각으로 자르면 94다.

따라서 위 "배칭 설계에 주는 답" 의 "관측된 피크 94는 임계 추정 하한(~95)보다 아래로
내려간다" 는 **조치 전 피크와 임계 하한이 사실상 같은 값(95 대 95)** 이라는 뜻으로
읽어야 한다. 문장의 논지("41% 절감이면 롤링 카운트가 같은 비율로 내려간다")는
유지되지만, 조치 **전** 값이 하한 아래라는 읽기는 성립하지 않는다.

#### 쿼터로 설명되지 않는 단발 거절 (2026-08-18 실측)

위 재측정이 "나머지 2건" 으로 남겨 뒀던 잔여를, 창을 08-03~08-18 로 넓혀
`check_vercel_quota.py` 로 다시 재 봤다. main 커밋 558 / 레코드 생성 529 /
non-head 4 / **거절 25**. 거절 25건은 두 무리로 깨끗하게 갈린다.

각 거절 시점의 24h 롤링 레코드 수 — 그 커밋이 겪은 쿼터 부하다. 세는 대상은
`[1]` 피크와 같은 **production+preview 전량**이고 창 규칙도 같은 `(t-24h, t]` 다:
...
정상 생성 529건의 롤링 24h 분포는 min 10 / p25 32 / **중앙값 42** / p75 51 /
max 94 다. 즉 단발 3건의 부하는 **정상 생성의 한가운데**에 있고, 버스트 22건은
전부 p75 를 한참 넘는다. 두 무리 사이에 45 | 78 의 빈 구간이 있다 — 같은 축의
정도 차이가 아니라 **다른 원인**이라고 읽어야 하는 모양이다.

> **max 94 는 위 "도구화" 절이 95 로 정정한 그 수치가 아니다.** 두 값은 평가 격자가
> 다르다. `[1]` 의 피크 95 는 **레코드** 시각에서 창을 끝내며 찾은 부하의 최대값이고
> (08-07 12:30:26.420), 여기 94 는 **커밋** 시각에서만 창을 끝낸 값의 최대다. 두 격자는
> 일치하지 않으므로 후자가 작은 것이 정상이다. 묻는 질문이 다르다 — "부하가 얼마까지
> 올라갔나" 대 "이 커밋이 겪은 부하가 얼마였나". 같은 도구가 둘 다 낸다.

단발 3건:

| 시각 (KST) | 커밋 | 롤링 24h | 다음 커밋까지 | 다음 커밋의 레코드 |
|---|---|---|---|---|
| 08-05 23:30:46 | `chore: collect regulatory news` | 45 | 449초 | 받음 |
| 08-09 02:00:12 | `chore: collect stock news` | 45 | 2281초 | 받음 |
| 08-14 19:44:20 | `chore: collect geopolitical risk data` | 37 | 11610초 | 받음 |

**배제되는 설명 세 가지.**

1. **쿼터가 아니다.** 위 분포 그대로다. 이 부하가 거절을 유발한다면 같은 부하 구간에
   놓인 정상 생성이 수백 건 남아 있을 수 없다. (임계를 결정적 상수로 가정하는 것은
   아니다 — 위 "식별 한계" 가 그걸 부정한다. 확률적 임계를 가정해도 부하 37~45 에서
   3건만 거절되고 수백 건이 통과하는 것은 쿼터로 설명되지 않는다.)
2. **푸시 batch non-head 가 아니다.** 다만 이 배제는 간격만으로는 성립하지 않는다 —
   셋 다 **다음 커밋이 레코드를 받았으므로**, `--batch-window` 를 449초 위로 올리면
   08-05 는 non-head 로 재분류된다. 배제의 근거는 간격이 아니라 **푸시 주체**다:
   세 쌍 모두 앞뒤가 서로 **다른 수집기 워크플로우**의 커밋이다(regulatory→stock,
   stock→crypto, geopolitical→coinmarketcap). 각 워크플로우는 자기 실행에서 자기
   커밋만 푸시하므로 한 푸시에 담길 수 없다. 08-09·08-14 는 간격만으로도 배제된다.
3. **경로 필터/`ignoreCommand` 가 아니다.** 바뀐 파일이 셋 다 다르다 — 08-05 는
   화이트리스트 `_state` 2개, 08-14 는 화이트리스트 **밖** `_state`, 08-09 는
   `_posts/` + `assets/` 가 든 완전한 콘텐츠 커밋이다. 애초에 `ignoreCommand` 는
   빌드만 건너뛰고 레코드는 `CANCELED` 로 남긴다(위 "실측 결과" 1번). 레코드가
   아예 없는 것은 이 경로로 설명되지 않는다.

**남는 설명과 그 한계.** 셋 다 `chore: collect …` 수집기 자동 커밋이지만 이건
증거가 아니다 — 창 안 main 커밋 558건 중 454건(81%)이 수집기 커밋이라, 3건이
우연히 전부 수집기일 확률이 **54%** 다. 가장 인색한 설명은 **GitHub → Vercel 연동의
간헐적 이벤트 유실**이다. Vercel 은 GitHub App 으로 붙어 있어 저장소 webhook
목록이 비어 있고(`gh api repos/.../hooks` → 0건), App 전달 로그는 우리 쪽에서
볼 수 없다. **이 절은 원인을 특정하지 못한다. 특정한 것은 원인이 쿼터가 아니라는
것뿐이다.**

**운영에 주는 답 두 가지.**

- **기저율은 3/558 = 0.5%.** 파일럿 후 거절 1건(08-14)은 이 기저율 안이므로
  no-op skip 파일럿의 회귀가 아니다. 반대로 거절 0건도 성공의 증거가 아니다 —
  `check_vercel_quota.py` 의 경고 문구가 이 뜻이다.
- **단발 거절은 포스트 유실이 아니다.** 커밋은 `main` 에 남아 있고 다음 배포가
  트리를 통째로 싣는다. 실제로 세 커밋의 산출물은 모두 현재 사이트에 있다.
  조치가 필요한 것은 "거절이 버스트로 몰릴 때" 뿐이고, 그 축은 롤링 누적이다.

재현:

```bash
python scripts/tools/check_vercel_quota.py \
  --since 2026-08-03 --pilot-merged 2026-08-10T13:14:51+09:00
```

거절별 부하와 정상 생성의 분포는 `[3]` 절이 직접 낸다 — 이 문서의 앞선 두 수치 오차
(91→79, 94→95)가 전부 손계산에서 나왔기 때문에, 이 절의 숫자는 손으로 만들지 않았다.
분포의 꼬리(min)는 창이 지나며 새 커밋이 들어와 하루 단위로 흔들린다. 위 표는
2026-08-18 11:02 KST 실행 기준이다.
