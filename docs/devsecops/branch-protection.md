...
건너뛴다(`_state/`·`.github/`·`tests/`·`docs/` 만 바뀐 경우). 오늘 40커밋 기준
**12/40 = 30%** 절감.
...
1. **command 필드는 256자 제한**(Vercel 스키마). 456자를 넣었다가 배포가 전부
   ...
   때문에 `scripts/` 제외 + postbuild 예외(305자)를 넣을 수 없어 절감이 45%→30%
   로 내려갔다.
2. **`:!<path>` bare pathspec 금지.** 경로 첫 글자에 따라 git 이 죽고, fail-open
   이라 그 fatal 이 "빌드"로 읽혀 조용하다. `:!./<path>` 를 쓴다.
3. **프리뷰 배포는 저장소 설정으로 막을 수 없다.** Vercel 은 배포를 생성한 뒤
   빌드만 건너뛰므로 배포 수 쿼터는 계속 소비된다. 끄려면 대시보드 설정이다.

4. **절감의 대상은 빌드 실행이지, 막고 있는 쿼터가 아니다.** 실측(2026-08-07):
   rate limit 에 걸린 커밋은 **배포 레코드가 아예 생성되지 않는다**
   (`GET /deployments` 로 확인 — 09:34 이후 Production 레코드 없음). 즉 Vercel 은
   `ignoreCommand` 를 평가하기 **전에** 거부한다. `ignoreCommand` 는 배포가 생성된
   뒤에만 돌므로, `Deployment rate limited`(일일 배포 생성 수)를 줄이지 못한다.
   빌드 실행 시간만 줄인다. 생성 후 skip 된 배포가 일일 한도에 계산되는지는
   대시보드 없이는 확인할 수 없다.

**결정: 일단 관찰.** 단 위 4번 때문에 관찰의 전제가 약하다 — 30% 는 빌드 시간
절감이고 지금 막고 있는 것은 배포 수 쿼터다. 쿼터가 계속 소진되면 실효 레버는
**프리뷰 비활성화(대시보드)** → **수집기 푸시 배칭(main 푸시 자체를 줄임)** →
**플랜 업그레이드** 순이며, 세 가지 모두 이번 변경 범위 밖이다.
