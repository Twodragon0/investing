...
건너뛴다(`_state/`·`.github/`·`tests/`·`docs/` 만 바뀐 경우). 실측 절감률은 아래
"실측 결과" 절을 볼 것 — 그날 수집기가 신규 항목을 얼마나 찾았는지에 따라 달라진다
(2026-08-07 추정 30%, 2026-08-08 실측 **56%**).
...
3. **프리뷰 배포는 저장소 설정으로 막을 수 없다** — 끄려면 대시보드 설정이다.
   프리뷰가 배포 수 쿼터를 소비하는지는 **미검증**이다(아래 실측은 Production
   환경만 다룬다).
4. **rate limit 에 걸린 커밋은 `ignoreCommand` 를 평가조차 하지 않는다.** Vercel 이
   배포 생성 단계에서 거부하므로, 이미 쿼터가 소진된 상태에서는 이 최적화가
   개입할 여지가 없다.

### 실측 결과 (2026-08-08) — skip 은 배포를 생성하지 않는다

2026-08-07 에 이 문서는 두 번 잘못 적혔다. 첫 버전은 "30% 절감으로 쿼터를
관찰한다"고 했고, 정정 버전은 "절감은 빌드 시간뿐이고 배포 수 쿼터는 줄지 않는다"고
했다. **둘 다 틀렸다.** 하루치 실측이 결론을 확정한다.

2026-08-08 main 커밋 9건을 `ignoreCommand` 규칙으로 분류하면 빌드 기대 4 / skip
기대 5 다. 같은 날 Production 배포 레코드는 **정확히 4건**이고 그 SHA 가 빌드 기대
4건과 **1:1로 일치**한다(`b8e42e38` `b0a71dba` `cac421c6` `3d8ac599`).

| 항목 | 값 |
|---|---|
| main 커밋 | 9 |
| 빌드 기대 → 실제 배포 레코드 | 4 → **4** |
| skip 기대 → 실제 배포 레코드 | 5 → **0** |
| 절감률(실측) | **56%** |

skip 된 커밋의 상태는 `success — Canceled by Ignored Build Step` 으로 보고된다
(예: docs 전용 머지 `e4c8faac`). 즉 **skip 은 배포 레코드를 만들지 않으므로 배포 수
쿼터도 소비하지 않는다.** 절감은 빌드 시간과 배포 수 **양쪽**에 적용된다.

절감률이 어제 추정한 30% 보다 높은(56%) 이유는 수집기가 신규 항목을 못 찾아
`_state/` 만 갱신하는 푸시의 비중이 그날그날 다르기 때문이다.

**남은 미검증**: PR 프리뷰 배포. 비-main 은 `ignoreCommand` 가 즉시 skip 하지만,
프리뷰가 배포 레코드를 만드는지는 Production 환경 목록으로 확인할 수 없다.
쿼터가 다시 소진되면 이쪽을 먼저 볼 것.

**결정: 관찰 계속.** 실효 절감이 56% 로 확인됐으므로, 이것으로 충분한지 며칠 더
지켜본다. 부족하면 프리뷰 비활성화(대시보드) → 수집기 푸시 배칭 → 플랜 업그레이드
순으로 검토한다.
