...
메인 워킹트리의 **파일**은 읽기만 한다(저장소 등록부 `.git/worktrees/` 에는 쓴다 — 9절). 관련 PR: #1347(락, 이후 철회) · #1348 · #1359 · #1360 · **#1361**(락·훅 철회 +
이 문서 승격).
...
## 0. 용어와 실행 방법 (처음 읽는다면 여기부터)

이 하네스는 **가드 테스트가 진짜로 무언가를 지키는지** 검사한다. 방법은 단순하다 —
지켜야 할 코드를 일부러 망가뜨리고(뮤테이션), 그 가드가 **red 가 되는지** 본다.
red 가 안 되면 그 가드는 통과하든 말든 아무것도 지키지 않는 것이다.

| 용어 | 뜻 |
|---|---|
| **StaticCase** | 케이스 하나 = (설명, 대상 파일, 바꿀 문자열 `old`, 바꿔 넣을 `new`, 확인할 pytest node id) |
| **앵커** | `old` 문자열. 대상 파일에 **정확히 1회** 나와야 한다 — 2회면 엉뚱한 곳을 바꾼다 |
| **샤드** | 케이스를 8등분한 것. CI 매트릭스가 병렬로 돌린다. 번호는 케이스 추가·삭제로 밀리므로 문서에 적지 않는다 |

판정은 뮤테이션을 넣었을 때(`patched`)와 원본(`control`)의 pytest 종료코드로 낸다:

| 판정 | 뜻 | 조치 |
|---|---|---|
| **FALSIFIABLE** | `patched≠0`, `control=0` — 가드가 제 일을 한다 | 없음 |
| **VACUOUS** | 둘 다 0 — **가드가 아무것도 안 지킨다** | 가드를 고친다 |
| **CONTROL-FAIL** | `control≠0` — 원본에서도 실패 | 환경·테스트 문제. 판정 불가 |
| **AMBIGUOUS-ANCHOR** | 앵커가 1회가 아님 | 앵커를 유일하게 만든다 |

> `patched_rc` 값을 봐라. **`1` 은 테스트 실패(가드가 잡음), `4` 는 pytest 수집
> 실패(주입한 코드가 깨짐)** 다. 후자를 "잡았다" 로 읽는 실수를 실제로 했다(12절).
> 지금은 `test_python_mutations_still_parse` 가 기계적으로 구별한다.

```bash
python3 scripts/tools/guard_falsifiability.py --check --shard 1/8   # CI 와 동일
python3 scripts/tools/guard_falsifiability.py --json                # 전수, JSON
python3 scripts/tools/guard_falsifiability.py --list-targets        # 트리거 경로
```

`--in-worktree` 는 **내부용**이다. 직접 쓰지 마라 — 격리를 건너뛰라는 뜻이 아니라
"이미 워크트리 안이다" 라는 재귀 방지 플래그고, 메인 트리에서 주면 즉시 죽는다.
...
분포는 `.github` 11 / `scripts` 10 / `tests` 10 / `.claude` 3 /
`.gitleaks.toml` 1 / `.pre-commit-config.yaml` 1 / `pyproject.toml` 1 이다(`_mutated_files()` — `guard_falsifiability.py:1367` 실행 결과).

그 구조에서 나온 사고, 전부 2026-09-18~19 실측:

| # | 사고 | 당시 완화 (2026-09-19) | **지금 (2026-09-23)** |
|---|---|---|---|
| A | 실행 중 커밋 → 뮤테이션이 커밋에 들어감 | #1347 락 + Claude 훅 | **해소.** 메인 트리를 변형하지 않는다 → 락·훅은 14절에서 걷음 |
| B | 스위트와 동시 실행 → bash 가 패치 중 스크립트를 이어 읽어 `exit 127` | 없음(사람이 조심) | **해소.** 스위트가 읽는 파일을 하네스가 건드리지 않는다 |
| C | SIGKILL 시 복원 실패 + git stat 캐시 잔상 | 없음 | **해소.** 워크트리와 함께 버려진다 — 15절에서 실측 |
...
`git rev-parse --absolute-git-dir` 로 고쳤다(커밋 `ee8990aa2`, 이후 #1348 로 머지). 이 수정
**없이는 이 계획을 시작할 수 없다.**

## 4. 착수 전 질문 Q1~Q6 — **전부 답났다**

> 아래 표는 2026-09-19 에 세운 질문 목록이고 "확인 방법" 열은 그때의 계획이다.
> **답은 9절(Q1·Q6)·10절(Q2·Q3·Q5)·이 절 아래(Q4)에 있다.** 표를 미해결 목록으로
> 읽지 말 것.
...
- **(b)** `_assert_safe_to_run()`(당시 존재, 2026-09-23 삭제)이 더러운 트리를
  거부하므로 "커밋 후 실행"
  을 규약으로 못박는다.
...
**5단계 — CI 전환.** ✅ **완료.** `guard-falsifiability.yml:145` 의 8샤드가 각자 워크트리를 만든다.
...
**6초 / 22,781 파일.** 4절 Q2·10절의 10.3초보다 빠르다(러너는 다를 수 있으니 5단계에서
...
**막으려던 사고는 구조적으로 불가능해졌다.** 범위를 정확히 적는다 —
하네스가 메인 저장소에 하는 일은 두 가지다:

| 대상 | 동작 |
|---|---|
| 워킹트리 **파일** | **읽기만** — `git diff HEAD`, `git ls-files`, 파일 복사 |
| `.git/worktrees/` **등록부** | **쓴다** — `git worktree add`/`remove --force`/`prune` (`cwd=REPO_ROOT`) |

락이 막던 사고는 **커밋에 뮤테이션이 섞이는 것**이고, 커밋이 담는 것은 워킹트리
파일이다. 등록부 쓰기는 커밋 내용에 들어가지 않으므로 그 사고와 무관하다.
(등록부 쓰기가 위험한 축은 따로 있다 — 11절의 워크트리 삭제 사고가 그 경로다.
거기는 락이 아니라 **접두사 필터와 폭발 반경 제한**이 지킨다.)

실행 중 커밋해도 워크트리는 이미 생성·이식을 마쳤으므로 런에 영향이 없고,
커밋에는 사람의 진짜 트리가 들어간다.
...
등록했다(FALSIFIABLE patched_rc=1). 설계 노트:

> 샤드 번호는 적지 않는다 — `STATIC_CASES` 의 round-robin 인덱스라 케이스가
> 추가·삭제될 때마다 밀린다. 실제로 등록 당시 8이었다가 커밋차단 케이스 삭제로
> 7로 옮겨갔다. 찾으려면 `select_shard()` 로 계산하라.
...
- **`ast.parse` 뮤테이션 검증** — §12 에서 권고했고 **2026-09-23 에 구현했다**
  (`tests/test_guard_falsifiability_tool.py::test_python_mutations_still_parse`).
  `.py` 를 겨냥한 모든 케이스의 뮤테이션 결과가 파싱되는지 확인한다.
  옛 버그를 재주입해 red 를 확인했다.
- **워크트리 생성 비용(5.8s/샤드) 절감** — 미착수. `--no-checkout` + sparse
  checkout 은 검토하지 않았다. 재사용은 10절 Q3 에서 기각했다(잔재 문제 재발).
- **5샤드 타임스탬프** — 13절의 Q2 실측은 8샤드 중 4개만 쟀다. 평균 5.8s 의
  분산을 보려면 나머지도 필요하다.
