...
#   - 수동 -> 사람이 기억해야 함
#
# 2026-09-20 갱신: 이 클론에 `pre-commit install` 을 했다. 위 첫 줄은 이제 이
# 클론에서는 거짓이다 — 하지만 **이 훅을 걷을 이유는 아니다.** 두 가지가 다르다:
#   - pre-commit 훅은 `git commit` 시점에 돈다. 이 훅은 **편집 시점**에 돈다.
#   - 다른 기여자·다른 클론은 여전히 미설치일 수 있다(설치는 추적되지 않는
#     `.git/hooks/` 에 산다).
#
